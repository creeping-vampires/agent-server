...
import { checkArbitrageOpportunity } from "./arbitrage/arbitrage";
...
export const arbitrageFunction = new GameFunction({
  ...
  executable: async (args, logger) => {
    try {
      ...
      // Create prompt for OpenAI
      const prompt = `Given the following BTC/USD price on HyperLiquid L1 Pre Reads:${l1Price} and BTC/USD price on HYPE EVM Pyth Oracle:${pythOraclePrice}:

      there is an arbitrage of ${priceDifferencePercentage}% with ${priceDifference} difference between the two prices. Please suggest weather there is an opportunity to capitalize on this arbitrage opportunity.`;
      ...
      return new ExecutableGameFunctionResponse(
        ExecutableGameFunctionStatus.Done,
        `Based on the current arbitrage of ${priceDifferencePercentage}% with ${priceDifference} difference between the two prices, here are some recommended activities:\n\n${recommendations}`
      );
    } catch (error) {
      return new ExecutableGameFunctionResponse(
        ExecutableGameFunctionStatus.Failed,
        `Failed to check arbitrage: ${
